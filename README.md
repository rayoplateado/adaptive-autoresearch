...
**Deterministic measurement.** Every metric has an executable script (`autodev-audit/metric-*.sh`) that outputs a single number. The agent never eyeballs a result — the scripts are the single source of truth. Run them yourself:

```bash
bash autodev-audit/run-all.sh
```
...
| File | Purpose |
|---|---|
| `autodev-audit/metric-*.sh` | Deterministic scripts measuring each quality metric |
| `autodev-audit/run-all.sh` | Runs all metrics, outputs JSON |
